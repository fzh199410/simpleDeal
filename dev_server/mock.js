...
const SKIP_MOCK = 'SKIP_MOCK';
const MOCK_HEADER_NAME = 'MOCK_INFO';
 ...
 * 并且如果json对象第一级数据配置了：SKIP_MOCK: true，则该直接跳过这次的mock
 * 
 * 定制具体的接口：
 * 以上面的为例：
...
function mockFactory(conf) {
    ...
    function mockMethod(req, res, next) {
        ...
        function mockRESTful(filePath) {
            ...
            res.set(MOCK_HEADER_NAME, 'MOCKED');

            if (jsFile) {
                return require(jsFile)(req, res, next);
            }
            if (jsonFile) {
                let jsonData = mockData(jsonFile);

                return jsonData[SKIP_MOCK] === true ? (res.set(MOCK_HEADER_NAME, 'SKIPED'), next()) : setTimeout(function(){res[sendMethod](jsonData);}, jsonData[TIME_OUT] || 0);
            }
            if (jsDir) {
                return require(jsDir)(req, res, next);
            }
            if (jsonDir) {
                let jsonData = mockData(jsonDir);
                return jsonData[SKIP_MOCK] === true ? (res.set(MOCK_HEADER_NAME, 'SKIPED'), next()) : setTimeout(function(){res[sendMethod](jsonData);}, jsonData[TIME_OUT] || 0);
            }

            res.set(MOCK_HEADER_NAME, 'NOT_FOUND');
