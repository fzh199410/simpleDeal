/*
 EzServerClient, 鏂规鍥介檯锛堝寳浜級鏈夐檺鍏徃. 
 V.7.0.0
*/
...
EzServerClient.Point.prototype = {
	 ...
	 * 2014.9.10
	 * @author qianleyi
	 * @description EzServerClient Point 澧炲姞鏂规硶
	 */
...
EzServerClient.Map = EzServerClient.Class.extend({
	...
	getPixelBounds: function () {
		 ...
		 * @author [qianleyi]
		 * @date [2014.7.18]
		 * @description [閻㈠彉绨珽zMapY閸ф劖鐖ｉ崥鎴烆劀閸栨绱濋惄瀛樺复add娴兼艾顕遍懛纰媧Map閸ф劖鐖ｆ潏鍦櫕鐠侊紕鐣婚柨娆掝嚖]
		 */
	...
	latLngToLayerPoint: function (latlng) { // (LatLng)
		...
		if (this.options.isEzMap) {
			/**
		 	* @author qianleyi
		 	* @description 閻㈠彉绨崸鎰垼閸樼喓鍋ｆ稉宥呮倱閿涘牊绨禒锝囩垳閸︺劌涔忔稉濠咁潡閿涘瓖zServerMap閸︼拷0,0閿涳拷
		 	*/
	...
	_resetView: function (center, zoom, preserveMapOffset, afterZoomAnim) {
		...
		if (!preserveMapOffset) {
			...
			if (this.options.isEzMap) {
				/**
			 	* @author [qianleyi]
			 	* @description [娣囶喗鏁糚an娴滃娆㈤崥搴ｇ級閺€鍙ョ皑娴犺泛顕遍懛瀵告畱閸︽澘娴橀崑蹇曅╅悳鎷岃杽閿涘本顒濆▎鈥叉叏閺€鍦畱BUG閸︹垥ap.js閻ㄥ垳latLngToNewLayerPoint閸戣姤鏆熸稉顓ㄧ礄824閿涘·BUG娣囶喗鏁肩粭顑跨癌婢跺垟
			 	*/
	...
	_getNewTopLeftPoint: function (center, zoom) {
		 ...
		 * @author [qianleyi]
		 * @Date [2014.7.18]
		 * @description [闁俺绻冪拋鍓х枂閸欏倹鏆熺拋鈺佸従閸掋倖鏌囬弰鐤巣Map閸︽澘娴橀敍灞绢劃閺冭泛娼楅弽鍢潪瀛樻Ц閸戝滑
		 */
		if (this.options.isEzMap) {
	...
	_latLngToNewLayerPoint: function (latlng, newZoom, newCenter) {
		if (this.options.isEzMap) {
			/**
		 	* @author [qianleyi]
		 	* @description [閸忓湧an娴滃娆㈤崥宸梒rollWheel缂傗晜鏂佺€佃壈鍤ч崷鏉挎禈娑撳﹣绗呴崑蹇曅BUG娣囶喗鏁肩粭顑跨婢跺垟
		 	*/
			 ...
			 * @author qianleyi
			 * @date 2014.9.9
			 * @description Marker鍦‥zMap鍧愭爣涓嬭烦鍔ㄩ棶棰�
			 */
	...
	setProxyOfEzMapService:function(proxyUrl){
		if (proxyUrl) {
			g_prox_calss = proxyUrl; // 鐠佸墽鐤嗗銈呯潣閹備簰娣囨繃瀵旀禒銉ュ閻楀牊婀伴崗鐓庮啇閹嶇礉
...
EzServerClient.TileLayer = EzServerClient.Class.extend({
	...
	_tileShouldBeLoaded: function (tilePoint) {
		...
		if (ezMap.MapCoordinateType !== 1 ) {return true;}
	...
	_adjustTilePoint: function (tilePoint) {
		...
		if (!this.options.continuousWorld && !this.options.noWrap) {
			 ...
			 * @author [qianleyi]
			 * @date [2014.7.18]
			 * @description [EzMap锛歺涓鸿礋鍊兼椂锛岄儴鍒嗙摝鐗囦笉鏄剧ず闂]
			 */
			if (!this._map.options.isEzMap) {
...
EzServerClient.TileLayer.EzMap = EzServerClient.TileLayer.extend({
	...
	_tileShouldBeLoaded: function (tilePoint) {
		...
		if (!options.continuousWorld) {
			...
			/**
			 * @author qianleyi
			 * 淇敼鍑虹幇璐熷€艰鍒楀彿鎶ラ敊鎯呭喌锛堢摝鐗囦笉鏄剧ず锛�
			 */
			if ((options.noWrap && (tilePoint.x >= limit.x)) ||
				 tilePoint.y >= limit.y) { return false; }
		}
...
EzServerClient.TileLayer.HotSpot = EzServerClient.TileLayer.extend({
	...
	_adjustTilePoint: function (tilePoint) {
		...
		if (!this.options.continuousWorld && !this.options.noWrap) {
			 ...
			 * @author [qianleyi]
			 * @date [2014.7.18]
			 * @description [EzMap閿涳拷娑撻缚锟斤拷鍏硷拷閿涳拷锟斤拷锟斤拷锟斤拷锟斤拷鍓с仛锟斤拷锟絔
			 */
			if (!this._map.options.isEzMap) {
				tilePoint.x = ((tilePoint.x % limit.x) + limit.x) % limit.x;
			}
	...
	_loadTile: function (tile, tilePoint) {
		...
		xhr.xhrObj.onreadystatechange = EzServerClient.bind(this._ajaxCallBack,xhr); //鍙戦€佷簨浠跺悗锛屾敹鍒颁俊鎭簡璋冪敤鍑芥暟
	...
	_createXHR: function() {
		...
		if (window.XMLHttpRequest) {
			...
			try{
				xmlHttpReq = new ActiveXObject("Msxml2.XMLHTTP");//IE楂樼増鏈垱寤篨MLHTTP  
			}
			catch(E){
				xmlHttpReq = new ActiveXObject("Microsoft.XMLHTTP");//IE浣庣増鏈垱寤篨MLHTTP 
			}
		}
...
EzServerClient.TileLayer.HotSpot2 = EzServerClient.TileLayer.extend({
    ...
    initialize: function(url, options, hotcallback) {
        ...
        this._hotspotFlag = true;

        this._hoturl = url + this.options.hot;

        this._url = url + this.options.tile;
        this._tileImage = EzServerClient.tileLayer(this._url, this.options);
        this._tileImage.setZIndex(50);
        this._tileImage._hotspotFlag = true;

        this._hotcallback = hotcallback;

        //璁剧疆name灞炴€х敤浜庡尯鍒簬澶氭簮鐑偣
    ...
    onAdd: function(map) {
        ...
        map['_hotSpotLayer' + this._name] = this;
    ...
    getHotSpotForTileLayer: function(isViewBounds) {
        ...
        for (var i in this._map['_hotspotareas' + this._name]) {
            var obj = this._map['_hotspotareas' + this._name][i],
                len = obj.length;
    ...
    _addHotSpot: function(e) {
        ...
        var hotspotareas = this._map['_hotspotareas' + this._name];
        if (tileID in hotspotareas) {
            var hotspotArr = hotspotareas[tileID],
                len = hotspotArr.length;
            for (var i = 0; i < len; i++) {
                var hotspotlatlng = EzServerClient.latLng(hotspotArr[i].Y, hotspotArr[i].X),
                    hotspotlayerpoint = this._map.latLngToLayerPoint(hotspotlatlng),
    ...
    onRemove: function(map) {
        ...
        map['_hotspotareas' + this._name] = null;
        ...
        if (!this.options.isMulti) {
            map.removeLayer(this._popupmarker);
        ...
        this._container = null;
        this._map = null;
        delete map['_hotSpotLayer' + this._name];
    },
    ...
    _reset: function(e) {
        this._map['_hotspotareas' + this._name] = {};
        if (!this.options.isMulti) {
            this._popupmarker.closePopup();
            this._popupmarker.setOpacity(0);
            this._marker.setOpacity(0);
        ...
        this._tilesToLoad = 0;
        this._map.fire("HOTSPOT:viewreset");
    },
    ...
    _tileShouldBeLoaded: function(tilePoint) {
        var hotspotareas = this._map['_hotspotareas' + this._name];
        if ((tilePoint.x + ':' + tilePoint.y) in hotspotareas) {
            return false; // already loaded
        }
    ...
    _addTile: function(tilePoint) {
        ...
        */
        ...
        var hotspotareas = this._map['_hotspotareas' + this._name];
        hotspotareas[tilePoint.x + ':' + tilePoint.y] = [];
    ...
    _ajaxCallBack: function() {
        ...
        if (xhr.readyState === 4) {
            if ((xhr.status >= 200 && xhr.status < 300) || xhr.status === 304) {
                var hotspotText = xhr.responseText;
                //澶勭悊鍥涘窛鐑偣鏁版嵁鏍煎紡涓嶆纭棶棰�
                var middleware = hotspotText; //hotspotText.replace(/(\"\"})/g,'\"}').replace(/(\/\>\"})/g,"}");
                var v = middleware.replace(/\r\n/g, "\r\n,").replace(/(\r\n)+,$/, "\r\n");
				//.replace(/\r\n/g, ",").replace(/(.\d+)$/, "");
				v = v.substr(0,v.length-1);
                eval("var hotspots=[" + v + "]");
                for (var i=0;i<hotspots.length;i++) {
                    hotspots[i]['TYPE'] = that._name;
					if(!that._map)continue;
					if(!(that._map['_hotspotareas' + that._name]&&that._map['_hotspotareas' + that._name][colAndRow])){
						that._map['_hotspotareas' + that._name][colAndRow]=[];
					}
                    that._map['_hotspotareas' + that._name][colAndRow].push(hotspots[i]);
                }
            }
        }
    },
...
EzServerClient.MultiHotSpot = EzServerClient.Class.extend({
    includes: EzServerClient.Mixin.Events,
    ...
    initialize: function(map, callback) {
        ...
        for (var i = 2; i < arguments.length; i++) {
            this.hotspotArr.push(arguments[i]);
        }

        this._map = map;

        this._hotcallback = callback;

        this.iconCache = {};

        this.selectArrayOfHotSpots = null;
        this.updateLayers();

        //瀹氫箟鐩稿叧鐨勭儹鐐逛簨浠�
        //
        this.viewrestCount = 1;

        map.on('HOTSPOT:viewreset', this._reset, this);
        map.on('mousemove', this._addHotSpot, this);

        this._marker = EzServerClient.marker([0, 0]).addTo(this._map);
        this._marker.setOpacity(0);

        this._popupmarker = EzServerClient.marker([0, 0]).addTo(this._map);
        this._popupmarker.setOpacity(0);
        ...
        this._marker.on('click', EzServerClient.bind(function(evt) {
            var latlng = this._marker.getLatLng();

            var newicon = this.iconCache['icon_' + evt.target._iconType];

            this._popupmarker.setIcon(newicon);
            this._popupmarker.setLatLng(latlng);
            this._hotcallback(this._popup);
            this._popupmarker.bindPopup(this._popup.getContent());
            this._popupmarker.openPopup();
        ...
        this._popupmarker.on('popupclose', EzServerClient.bind(function() {
            ...
            this._popupmarker.unbindPopup();
            this._popupmarker.setOpacity(0);
            this._popupmarker.setLatLng([0, 0]);
        }, this));
    },

    updateLayers: function() {
        this.selectArrayOfHotSpots = this.hotspotArr.map(function(val) {
            var type = val.options.name;
            var icon = val.options.icon;
            this.iconCache['icon_' + type] = icon;
            return this._map['_hotspotareas' + type];
        }, this);
    },

    _reset: function() {
        var len = this.hotspotArr.length;
        if (this.viewrestCount !== len) {
            this.viewrestCount++;
            return;
        }
    ...
    removeHotSpotLayer: function(layer) {
        for (var i = 0; i < this.hotspotArr.length; i++) {
            if (layer.options.name === this.hotspotArr[i].options.name) {
                this.hotspotArr.splice(i, 1);
                break;
            }
        }
        this.updateLayers();
    ...
    addHotSpotLayer: function(layer) {
        this.hotspotArr.push(layer);
        this.updateLayers();
        this.viewrestCount = 1;
    },
	...
	_addHotSpot: function(e) {
        ...
        for (var j = 0; j < this.selectArrayOfHotSpots.length; j++) {
            var hotspotareas = this.selectArrayOfHotSpots[j];

            //澶氭簮鐑偣澧炲姞闈瀕en鏃剁殑褰撳墠婧愭爣璇�
            var isHotSpotFlag = false;
				
            if (hotspotareas && tileID in hotspotareas) {
                var hotspotArr = hotspotareas[tileID],
                    len = hotspotArr.length;
                for (var i = 0; i < len; i++) {
                    var hotspotlatlng = EzServerClient.latLng(hotspotArr[i].Y, hotspotArr[i].X),
                    ...
                    if (hotspotBounds.contains(containerPoint)) {
                        ...
                        this._marker.setLatLng(hotspotlatlng);
                        this._marker.setIcon(this.iconCache['icon_' + hotspotArr[i].TYPE]);
                        this._marker._iconType = hotspotArr[i].TYPE;
                        this._marker._icon.title = hotspotArr[i].LABEL;
                        this._marker.setOpacity(1);
                        this._popup.contentData = data;
                        break;
...
EzServerClient.Marker = EzServerClient.Class.extend({
	...
	_onMouseClick: function (e) {
		...
		if ((!this.dragging || !this.dragging._enabled) &&this._map&& this._map.dragging && this._map.dragging.moved()) { return; }
...
EzServerClient.Polygon = EzServerClient.Polyline.extend({
	...
	triangleArea: function(p0,p1,p2) {
		var area = 0;
	...
	getCenter: function() {
		...
		var garr = [];
		var p0  = new EzServerClient.Point(arr[0].lng,arr[0].lat);

		for (var i = 0; i < arr.length - 2; i++) {
			var p1 = new EzServerClient.Point(arr[i + 1].lng,arr[i + 1].lat);
			var p2 = new EzServerClient.Point(arr[i + 2].lng,arr[i + 2].lat);
			var area = this.triangleArea(p0,p1,p2);
			var center = new EzServerClient.Point((p0.x + p1.x + p2.x)/3,(p0.y + p1.y + p2.y)/3);
			garr.push({
				"center":center,
				"area":area
			});
		...
		for (var i = 0; i < garr.length; i++) {
			gx += garr[i].center.x * garr[i].area;
			gy += garr[i].center.y * garr[i].area;
			garea += garr[i].area;
		}

		return [gx/garea,gy/garea];
	},
...
/**
 * @fileOverview QueryObject绫�
 * @author <a href="http://www.easymap.com.cn">鍖椾含灞辨捣缁忕含淇℃伅鎶€鏈湁闄愬叕鍙�</a>
 * @version 6.3.4
 */
QueryObject = EzServerClient.Class.extend({
	options: {
		...
		serviceSource: "",										//璁剧疆杩滅▼璁块棶EzMapServerice鍦板潃
		precision: 6,											//璁剧疆鏌ヨ缁撴灉涓璖hape绫诲瀷Double鍊肩殑鏈夋晥灏忔暟鐐逛綅鏁�
		baseDistanceTolerance: 0.00001,							//鏌ヨ缁撴灉涓�,鍑犱綍瀛楁鍧愭爣杩斿洖鐨勬娊绋€闃堝€�
		orderByClause: "", 										//璁剧疆鏌ヨ鐨刼rder by鏉′欢				
		isFieldsContainDispField: true							//鐢ㄤ簬鍒ゆ柇鏌ヨ瀛楁鍒楄〃涓腑鏄惁鍖呭惈鏄剧ず瀛楁
	},
	...
	/**
	 * @description 鍑芥暟鍔熻兘锛氳缃煡璇㈠拰杩斿洖鐨勫瓧娈靛€�
	 * @param {String}
	 *            strFields <font color=orange
	 *            size=2>鍙傛暟鍚箟锛氳缃煡璇㈠拰杩斿洖鐨勫瓧娈靛€�,濡�:"瀛楁鍚�1:鏄剧ず鍚�1;瀛楁鍚�2:鏄剧ず鍚�2"<br>
	 *            鍙傛暟鐪佺暐锛氫笉鍙互鐪佺暐</font>
	 * @return {Void}
	 */
	addSubFields: function(strSubfield) {
		try {
			...
			this.queryFields = this.fields.slice(0); 			//鐢ㄤ簬鍒ゆ柇鏌ヨ瀛楁鍒楄〃涓槸鍚﹀寘鍚樉绀哄瓧娈�
			}
			catch (e) {
	...
	/**
	*@description [鑾峰彇鏌ヨ瀵硅薄鎵€璁惧畾鐨凟zMapService璇锋眰鍦板潃锛屾湭璁惧畾鏃朵负榛樿EzMapService鎵€鎸囧畾鐨勫湴鍧€ return]
 	*@param {string}  [杩斿洖鍚慐zMapService鎵€璇锋眰鐨刾ost鍦板潃] 
	*/
	getPostURL:function(){
		if (this.options.serviceSource == null || this.options.serviceSource == '') {
		// 杩斿洖EzMapAPI.js閰嶇疆涓瀹氱殑榛樿EzServerClient.GlobeParams.EzMapServiceURL鐨刾ost鐨刄RL
		return EzServerClient.GlobeFunction.formatStrURL('');
		} 
		else {
		// 杩斿洖鎸囧畾EzMapService鐨勭┖闂存煡璇RL
	 ...
	 * 鍑芥暟璇存槑锛氱敓鎴愭煡璇㈢殑xml
	 * 
	 * @return {String} 杩斿洖鍚慐zMapService璇锋眰鐨剎ml
	 * @private
	 */
	toxml:function(){
	...
	/**
	 * 鍑芥暟鍔熻兘锛氱敓鎴愭ā绯婃煡璇㈢殑XML
	 * 
	 * @return {String} 杩斿洖妯＄硦鏌ヨ鐨凟ASYXML1.1瀛楃涓�
	 * @private
	 */
	getFuzyQueryXML: function() {
		...
		uSelect.setAlwaysReturnShape(true);// 璁剧疆鏄惁寮哄埗杩斿洖鍑犱綍瀛楁
	 ...
	 * 鍑芥暟鍔熻兘锛氱敓鎴愮偣鏌ヨ鐨刋ML
	 * 
	 * @return {String} 杩斿洖鐐规煡璇㈢殑EASYXML1.1瀛楃涓�
	 * @private
	 */
	...
	/**
	 * 鍑芥暟鍔熻兘锛氱敓鎴愭媺妗嗘煡璇㈢殑XML
	 * 
	 * @return {String} 杩斿洖鎷夋鏌ヨ鐨凟ASYXML1.1瀛楃涓�
	 * @private
	 */
	...
	/**
	 * 鍑芥暟鍔熻兘锛氱敓鎴愬渾褰㈡煡璇㈢殑XML
	 * 
	 * @return {String} 杩斿洖鍦嗗舰鏌ヨ鐨凟ASYXML1.1瀛楃涓�
	 * @private
	 */
	...
	/**
	 * 鍑芥暟鍔熻兘锛氱敓鎴愬杈瑰舰鏌ヨ鐨刋ML
	 * 
	 * @return {String} 杩斿洖澶氳竟褰㈡煡璇㈢殑EASYXML1.1瀛楃涓�
	 * @private
	 */
	...
	/**
	 * 鍑芥暟鍔熻兘锛氱敓鎴愬懆杈规煡璇㈢殑XML
	 * 
	 * @return {String} 杩斿洖鍛ㄨ竟鏌ヨ鐨凟ASYXML1.1瀛楃涓�
	 * @private
	 */
...
//		uSelect.setSpatialFilters(uSpatialFilters);
	...
	/**
	 * 鍑芥暟鍔熻兘锛氱敤浜庡垽鏂樉绀哄瓧娈典笌瑕佽繑鍥炵殑瀛楁闂存槸鍚﹂噸澶�
	 * 
	 * @return {Boolean} 杩斿洖鏄剧ず瀛楁鏄惁鍦ㄦ煡璇㈠瓧娈典腑
	 * @private
	 */
	...
	/**
	 * 鍑芥暟鍔熻兘锛氭牴鎹畇hape绫诲瀷涓嶅悓鏉ユ嫾鎺ASYXML1.1瀛楃涓�
	 * 
	 * @param {String}
	 *            vShapType 绌洪棿杩囨护绫诲瀷
	 * @param {Float}
	 *            vBufferSize 鐐规煡璇㈠拰鍛ㄨ竟鏌ヨ缂撳啿鍖哄崐寰�
	 * @return {String} 杩斿洖鐐规煡璇㈢殑EASYXML1.1瀛楃涓�
	 * @private
	 */
	...
	query: function(vQuery, vCallback) {
		try{
			if(!EzServerClient.GlobeFunction.isTypeRight(vQuery, "QueryObject") && !EzServerClient.GlobeFunction.isTypeRight(vQuery, "Array")){
				throw EzErrorFactory.createError("EzMap::query鏂规硶涓璦rguments[0]is not");
			}
			if(!EzServerClient.GlobeFunction.isTypeRight(vCallback, "function")){
				throw EzErrorFactory.createError("EzMap::query鏂规硶涓璦rguments[1]is not");
			}
			//EzMapService鏌ヨ鍗忚鍙樺寲鍚庯紝beginRecord鐢�0鍙樻垚1
			...
			// 璁剧疆褰撲竴缁勬煡璇㈠叾涓竴涓嚭閿欐椂锛屾槸鍚︾粓姝㈡煡璇�
        	...
        	uEzMapServiceQuery.onFinished = function(){
				for(var i=0;i<uEzMapServiceQuery.getQueryItemCount();i++){
					...
					// 鍏煎鎬ф祴璇�
					uEzLayer.layerId = uQuery.options.layerId;// 璁剧疆
					uEzLayer.tableName = uQuery.options.tableName;// 璁剧疆
					uEzLayer.layerName = uQuery.options.layerName;// 璁剧疆
					uEzLayer.queryObj = uQuery;// 璁剧疆
					uEzLayer.imgURL = uQuery.options.imgURL// 璁剧疆
					...
					if(uError){
							...
							for(var j=0; j<uEzLayer.recordCount; j++){
								...
								for(var m=0; m<uFields.size(); m++){
									...
									if(uEzField.name == uQuery.options.dispField.toUpperCase()){
										...
										uFeatureObject.dispname = uFeatureObject.dispFieldValue;// V6.3.1鍙婂叾浠ュ墠鐗堟湰dispname灞炴€у懡鍚嶈瘝涓嶈揪鎰忥紝鐜扮敤dispFieldValue鏇夸唬
										// 濡傛灉鏌ヨ瀛楁涓笉鍖呭惈鏄剧ず瀛楁锛屽垯姝ゆ樉绀哄瓧娈靛€间笉鍔犲叆鍒版樉绀虹粨鏋滈泦鍚堜腑
										if (!uQuery.isFieldsContainDispField) {
											continue;
										}
									}
								
									// 涓哄吋瀹逛互鍓嶇殑FeatureOjbect,璁剧疆FeatureObject涓殑鐩稿叧灞炴€�
						...
						// 鍜孍zMapService杩炴帴鏈夐棶棰樼殑鎯呭喌
						uEzLayer.error = new Error("EzMapService not");
					}
				}
				// 璋冪敤浜屾寮€鍙戠敤鎴风殑鍥炶皟鍑芥暟
...
//		var uQueryResults = this.getQueryResults2().Clone();// getQueryResults2涓鸿幏鍙栫┖闂存煡璇㈢粨鏋滅殑鍏嬮殕
		var uQueryResults = this.getQueryResults2().slice(0);
		_LayerArr = uQueryResults;// 涓轰簡淇濇寔浠ュ墠鐗堟湰鍏煎鎬�
...
EzErrorFactory = EzServerClient.Class.extend({
	statics:{
		/**
		 *<font color=orange size=2>鍑芥暟鍔熻兘锛氬垱寤洪敊璇璞�
	 	 *@param {String} message 鍙傛暟鍚箟锛氫笌閿欒鐩歌仈鐨勬暟瀛楀€�
		 *鍙傛暟鐪佺暐锛氫笉鍙互鐪佺暐
	 	 *@param {Error} cause 鍙傛暟鍚箟锛氬綋鍓嶉敊璇璞＄殑涓婁竴绾ч敊璇璞�
		 *鍙傛暟鐪佺暐锛氬彲浠ョ渷鐣�</font><br>
		 *@return {Error}
		 *2009.07.27
		 */
...
EzServerClient.GlobeFunction = EzServerClient.Class.extend({
	statics:{
		/**
		 *<font color=orange size=2>鍑芥暟鍔熻兘锛氬垽鏂弬鏁皃aram1绫诲瀷鏄惁鏄痶ype绫诲瀷
		 *@param {Object} param1 鍙傛暟鍚箟锛氬緟鍒ゆ柇鐨勫璞�
		 *鍙傛暟鐪佺暐锛氫笉鍙互鐪佺暐
		 *@param {String|Object} type 鍙傛暟鍚箟锛氱洰鏍囩被鍨�
		 *鍙傛暟鐪佺暐锛氫笉鍙互鐪佺暐</font><br>
		 *@return {Boolean}
		 *2009.07.27
		 */
		...
		/**
		 * 鍑芥暟鍔熻兘锛氬綋鍓嶇粨鏋滈泦鍚堜腑鏄惁宸茬粡鏈変簡姝ゅ浘灞�
		 * @return {Boolean}
		 * @private
		...
		/**
		 * 鍑芥暟鍔熻兘锛氳幏寰梡ost璇锋眰鐨剈rl鍦板潃鍜寈ml瀛楃涓叉墍缁勬垚鐨勫璞�
		...
		getRequestObject:function(vQuery){
			...
			//qianleyi
			//锟芥壆锟斤拷锟借礋濮濓拷锟藉鑼拷锟斤拷锟絏ML锟斤拷銆傜€电钖�
			// 婵★拷锟斤拷锟藉敩锟斤拷锟斤拷锟斤拷娑擄拷锟界拋鍓х枂锟斤拷鍞拷锟斤拷锟铰わ拷濮癸拷锟絬rl锟斤拷ost锟斤拷锟界粭锟借
	...
	dealWithRequest: function(pRequestObj, callBack) {
		...
		try {
			...
			if(pRequestObj instanceof QueryObject || pRequestObj instanceof EditObject) {
				//鏌ヨ娆℃暟
				this._EzServerClientQueryNum = 1;
				// 璁剧疆鍐呴儴鍥炶皟鍑芥暟鏄煡璇㈠璞¤繕鏄紪杈戝璞★紝pCallBack涓哄唴閮ㄥ洖璋冨嚱鏁帮紝鐢ㄦ潵瑙ｆ瀽鏌ヨ鑾风紪杈戠殑XML
...
EzServerClient.FeatureObject = EzServerClient.Class.extend({
	initialize: function(uLayer) {
		...
		if (uLayer == null) {
			throw new Error("锟斤拷锟节癸拷锟届函锟斤拷锟叫达拷锟斤拷图锟斤拷锟斤拷锟斤拷!")
		}
		this.layInfo = uLayer;

		// 20091225锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷
		/**
		 * @description <font color=orange size=2>锟斤拷锟皆猴拷锟藉：要锟截碉拷锟街段讹拷锟襟集猴拷<br>
		 *              锟斤拷始默锟斤拷值锟斤拷[]</font><br>
		 * @return {Array}
		 * @field
		 * @private
		 */
		this.ezFields = [];
		/**
		 * @description <font color=orange size=2>锟斤拷锟皆猴拷锟藉：要锟截碉拷锟斤拷锟侥碉拷<br>
		 *              锟斤拷始默锟斤拷值锟斤拷null</font><br>
		 * @return {Point}
		 * @field
		 * @private
		 */
		 ...
		 *              size=2>锟斤拷锟皆猴拷锟斤拷锟斤拷示锟街讹拷锟斤拷锟斤拷应锟斤拷值锟斤拷this.dispname锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟绞诧拷锟斤拷锟解，锟斤拷V6.3.5锟斤拷锟斤拷锟皆猴拷锟芥本锟斤拷dispFieldValue锟斤拷锟斤拷锟斤拷锟斤拷<br>
		 *              锟斤拷始默锟斤拷值锟斤拷锟斤拷</font><br>
		 * @return {String}
		 * @field
		 * @private 20091103
		 */
	...
	getMBR:function(){
		console.log("锟芥本锟斤拷锟斤拷锟捷ｏ拷未锟睫改ｏ拷")
		// var pMBR = EzServerClient.GlobeFunction.getPathMBR(this.linestr);// 锟斤拷全锟街猴拷锟斤拷锟斤拷initalize.js锟叫讹拷锟斤拷
		// return pMBR;
	},
	...
	setCenterPoint:function(vPoint){
		try {
			if (!(EzServerClient.GlobeFunction.isTypeRight(vPoint, EzServerClient.Point)))
				throw EzErrorFactory.createError("FeatureObject::setCenterPoint锟斤拷锟斤拷锟斤拷arguments[0]锟斤拷锟酵诧拷锟斤拷确");
			else {
				this.centerPoint = vPoint;
				this.point = this.centerPoint;
			}
		} catch (e) {
			throw EzErrorFactory.createError("FeatureObject::setCenterPoint锟斤拷锟斤拷锟叫诧拷锟斤拷确",e);
		}
	},
		...
		try {
			if (!EzServerClient.GlobeFunction.isTypeRight(vEzShapeArr, "Array")) {
				throw EzErrorFactory
						.createError("EzLayer::setSpatialFilterShapes锟斤拷锟斤拷锟斤拷arguments[0]锟斤拷锟斤拷锟斤拷锟酵诧拷锟斤拷确");
			}
			this.spatialFilterShapes = vEzShapeArr;
		} catch (e) {
			throw EzErrorFactory.createError(
					"EzLayer::setSpatialFilterShapes锟斤拷锟斤拷执锟叫诧拷锟斤拷确", e);
		}
	},
	...
	setSpatialColumn:function(vSpatialColumn){
		try {
			if (!EzServerClient.GlobeFunction.isTypeRight(vSpatialColumn, "string")) {
				throw EzErrorFactory.createError("EzLayer::setSpatialColumn锟斤拷锟斤拷锟斤拷arguments[0]锟斤拷锟斤拷锟斤拷锟酵诧拷锟斤拷确");
			}
			this.spatialColumn = vSpatialColumn;
		} catch (e) {
			throw EzErrorFactory.createError("EzLayer::setSpatialColumn锟斤拷锟斤拷执锟叫诧拷锟斤拷确", e);
		}
	},
	...
	setGeometryType:function(vGeometryType){
		try {
			if (!(EzServerClient.GlobeFunction.isTypeRight(vGeometryType, "string"))) {
				throw EzErrorFactory
						.createError("EzLayer::setGeometryType锟斤拷锟斤拷锟斤拷arguments[0]锟斤拷锟酵诧拷锟斤拷确");
			} else {
				this.geometryType = vGeometryType;
			}
		} catch (e) {
			throw EzErrorFactory.createError("EzLayer::setGeometryType锟斤拷锟斤拷锟叫诧拷锟斤拷确", e);
		}
	},
	...
	setFeatures:function(vFeatures){
		try {
			if (!(EzServerClient.GlobeFunction.isTypeRight(vFeatures, "Array"))) {
				throw EzErrorFactory
						.createError("EzLayer::setFeatures锟斤拷锟斤拷锟斤拷arguments[0]锟斤拷锟酵诧拷锟斤拷确");
			} else {
				this.features = vFeatures;
			}
		} catch (e) {
			throw EzErrorFactory.createError("EzLayer::setFeatures锟斤拷锟斤拷锟叫诧拷锟斤拷确", e);
		}
	}
});
		...
		try{
			...
			uHttpRequest.onreadystatechange=function(){
				if(uHttpRequest.readyState == 4){
					...
					if (vClientCallback){
						// 濡傛灉鏄崟涓煡璇紝鍒欑珛鍗虫墽琛屼簩娆″紑鍙戠敤鎴风殑鍥炶皟鍑芥暟
			...
			//姝ゅ鍚炴帀杩炴帴鍦板潃涓嶅瓨鍦ㄦ垨杩炴帴寮傚父鏃剁殑寮傚父
...
EzServerClient.EzShape = EzServerClient.Class.extend({
	...
	setGeometryType:function(vGeometryType){
		try{
			if( !(EzServerClient.GlobeFunction.isTypeRight(vGeometryType, "string"))){
				throw EzErrorFactory.createError("EzShape::setGeometryType锟斤拷锟斤拷锟斤拷arguments[0]锟斤拷锟酵诧拷锟斤拷确");
			}else{
				this.geometryType = vGeometryType;
			}
		}catch(e){
			throw EzErrorFactory.createError("EzShape::setGeometryType锟斤拷锟斤拷锟叫诧拷锟斤拷确", e);
		}		
	},
	...
	setCoordinateSequence:function(vCoordinateSequence){
		try{
			if( !(EzServerClient.GlobeFunction.isTypeRight(vGeometryType, "string"))){
				throw EzErrorFactory.createError("EzShape::setCoordinateSequence锟斤拷锟斤拷锟斤拷arguments[0]锟斤拷锟酵诧拷锟斤拷确");
			}else{
				this.coordinateSequence = vCoordinateSequence;
			}
		}catch(e){
			throw EzErrorFactory.createError("EzShape::setCoordinateSequence锟斤拷锟斤拷锟叫诧拷锟斤拷确", e);
		}
	}
});

Circle = EzServerClient.Class.extend({
	initialize : function(points, color, weight, opacity, fillcolor,options) {
		...
		if (options) {
			...
			newOptions['weight'] = weight;
			newOptions['fillOpacity'] = opacity;
			newOptions['fillColor'] = fillcolor;		
		}

		this.uCircle = EzServerClient.circle(circleCenter, radius, newOptions);
	},
	
	addTo:function(map){
		this.circleFlag = true;
		this.uCircle['circleFlag'] = true;
		this.uCircle.addTo(map);
	},

	addListener: function(action, func) {
		this.uCircle.on(action, func);
	},
	...
	openInfoWindowHtml:function(strHTML,options){
		var popup = new EzServerClient.Popup(options);
		popup.setContent(strHTML);
		this.uCircle.bindPopup(popup).openPopup();
	},
	
	closeInfoWindowHtml:function(){
		this.uCircle.closePopup();
...
//Client鏂规硶澧炲姞鍒發eaflet Point涓�107~126

Polyline = EzServerClient.Class.extend({
    initialize: function(points, color, weight, opacity, options) {
        ...
        if (options) {
            ...
            newOptions['color'] = color;
            newOptions['weight'] = weight;
            newOptions['opacity'] = opacity;
        }

        this.uPolyline = new EzServerClient.Polyline(latlngs, newOptions);
    },
    ...
    openInfoWindowHtml: function(strHTML, options) {
        var popup = new EzServerClient.Popup(options);
        popup.setContent(strHTML);
        this.uPolyline.bindPopup(popup).openPopup();
    },
    ...
    addTo: function(map) {
        ...
        this.uPolyline['polylineFlag'] = true;
        this.uPolyline.addTo(map);
    },

    addDecorator: function(options) {
        this.arrowPoly_ = EzServerClient.polylineDecorator(this.uPolyline, options);
        this.arrowPoly_.addTo(this.map_);
    },

    generateContent: function(options) {
        var triangle = 'M0 0L10 5L0 10 z';
        var width = options.width || 10;
        var height = options.height || 10;
        var viewBox = '0 0 ' + width + ' ' + height;
        var fill = options.fill || "#000000";

        var content = "<marker id='arrowHead' viewBox='" + viewBox + "' markerWith='" + width + "' markerHeight='" + height + "' refx='1' refy='5' orient='auto'><path d='" + triangle + "' fill='" + fill + "'/></marker>";

        return content;
    },

    addArrowHead: function(options) {
        var path = this.uPolyline.getPath(),
            containter = this.uPolyline.getContainer();

        var content = this.generateContent(options);
        var defs = this.uPolyline._createElement('defs');
        defs.innerHTML = content;
        containter.appendChild(defs);
    ...
    toWKT: function() {
        var latlngs = this.uPolyline.getLatLngs(),
            latlngString = "";
        for (var i = 0; i < latlngs.length; i++) {
            latlngString += latlngs[i].lng + " " + latlngs[i].lat;
            if (i != latlngs.length - 1) {
                latlngString += ',';
            }
        }

        latlngString = 'LINESTRING(' + latlngString + ')';
        return latlngString;
    },

    toCoords: function(wkt) {
        if (!wkt) {
            return;
        }
        // var coords = wkt.substring(wkt.indexOf('(') + 2, wkt.indexOf(')'));
        // 姝ｅ垯琛ㄨ揪璁＄畻鎷彿鍐呯殑瀛楃涓�
        var coordsArray = wkt.match(/(\([\d\s,.]+\))/g);
        var newcoords = '';
        for (var i = 0; i < coordsArray.length; i++) {
            var coords = coordsArray[i].substring(1, coordsArray[i].length - 1);
            var coordArr = coords.split(',');
            for (var j = 0; j < coordArr.length; j++) {
                var lnglat = coordArr[j].split(' ');
                newcoords += lnglat[0] + ',' + lnglat[1];

                if (j != coordArr.length - 1) {
                    newcoords += ',';
                }
            }
    ...
    _getLngRadius: function(lat, radius) {
        return this._getLatRadius(radius) / Math.cos(EzServerClient.LatLng.DEG_TO_RAD * lat);
    },

    toBuffer: function(radius) {
        if (!jsts.io) {
            console.error("闇€瑕佸紩鍏STS鍖�");
        }
        var data = this.toWKT();
        ...
        var geoReader = new jsts.io.WKTReader();
        var geoWriter = new jsts.io.WKTWriter();
        var geometry = geoReader.read(data).buffer(this._getLngRadius(firstPoint.lat, radius));
        return this.toCoords(geoWriter.write(geometry));
    }
});
...
Polygon = EzServerClient.Class.extend({
    initialize: function(points, color, weight, opacity, fillcolor, options) {
        ...
        for (var i = 0; i < arrLatLngArr.length; i++) {
            ...
            for (var j = 0; j < arrLatLngArr.length; j++) {
                ...
                if (this._isInArr(sortIndexArr, j)) {
                    continue;
                }
            }
        }
        ...
        this.uPolygon = new EzServerClient.Polygon(arrLatLngArr, newOptions);
    },
    ...
    openInfoWindowHtml: function(strHTML, options) {
        var popup = new EzServerClient.Popup(options);
        popup.setContent(strHTML);
        this.uPolygon.bindPopup(popup).openPopup();
    },
    ...
    addTo: function(map) {
        this.polygonFlag = true;
        this.uPolygon['polygonFlag'] = true;
    ...
    getCenter: function() {
        return this.uPolygon.getCenter();
    },
    ...
    toWKT: function() {
        ...
        latlngString += latlngs[0].lng + " " + latlngs[0].lat;

        latlngString = 'POLYGON((' + latlngString + '))';
        return latlngString;
    },
...
Rectangle = Polygon.extend({
	initialize: function(points, color, weight, opacity, fillcolor,options) {
		...
		this.uRect = new EzServerClient.Polygon(bounds, newOptions);
	},

	addTo:function(map){
		this.rectFlag = true;
		this.uRect['rectFlag'] = true;
	...
	openInfoWindowHtml: function(strHTML,options) {
		var popup = new EzServerClient.Popup(options);
		popup.setContent(strHTML);
		this.uRect.bindPopup(popup).openPopup();
	},
...
EzMap = EzServerClient.Class.extend({
    statics: {
        ...
        /**
         * [initialize description]
         * @param  {a,b} id [锟斤拷锟诫经纬锟斤拷锟斤拷锟疥（锟叫癸拷锟斤拷习锟竭ｏ拷]锟斤拷锟斤拷锟斤拷/[a,b]/a,b
         * @return {[point]}    [latlng(锟斤拷锟斤拷锟斤拷习锟斤拷)]
         */
    ...
    initialize: function(id) {
        ...
        //默锟斤拷锟斤拷锟斤拷一锟斤拷锟斤拷片图锟斤拷
        //通锟斤拷锟斤拷锟矫诧拷锟斤拷锟斤拷锟斤拷图锟斤拷锟斤拷锟斤拷
        ...
        if (layers.length > 1) {
            ...
            ezMapLayers.tileLayerObject = EzServerClient.tileLayer(layers[0][1], crs);
    ...
    stringToCrs: function(jsonObj) {
        ...
        for (var i in jsonObj) {
            if (i === "crs") {
                ...
                newObj[i] = newcrs;
            } else if (i === "annotation") {
                newObj['annotationLayer_'] = true;
            } else {
                newObj[i] = jsonObj[i];
            }
        }

        if (newObj['annotationLayer_']) {
            var annotationLayer = EzServerClient.tileLayer(jsonObj['annotation'], newObj);
            newObj['annotationLayer_'] = annotationLayer;
        }
        return newObj;
    },
            ...
            if (ezMap.layersControlStyle === 1) {
                ...
                for (var i in this.ezMapLayers) {
                    if (i === layers[0][0]) {
                        ...
                        if (this.ezMapLayers[i].options.annotationLayer_) {
                            this.uMap.addLayer(this.ezMapLayers[i].options.annotationLayer_);
                            this.ezMapLayers[i].options.annotationLayer_.bringToFront();
                        }
                        break;
                    }
                }
                ...
                var layers = ezMap.MapSrcURL;
                for (var i in this.ezMapLayers) {
                    var layer = this.ezMapLayers[i];
                    EzServerClient.layersButton(i, this.uMap, layer);
                    if (i === layers[0][0]) {
                        this.uMap.addLayer(layer);
                        if (this.ezMapLayers[i].options.annotationLayer_) {
                            this.uMap.addLayer(this.ezMapLayers[i].options.annotationLayer_);
                            this.ezMapLayers[i].options.annotationLayer_.bringToFront();
                        }
    ...
    changeDragMode: function(drawMode, callback, state, obj) {
        if (this.modeStatus) {
            switch (this.modeStatus) {
                case "drawPoint":
                    this.uMap.fire('destruct::drawPoint');
                    break;
                case "measure":
                    this.uMap.fire('destruct::measure');
                    break;
                case "drawPolyline":
                    this.uMap.fire('destruct::drawPolyline');
                    break;
                case "drawRect":
                    this.uMap.fire('destruct::drawRect');
                    break;
                case "drawPolygon":
                    this.uMap.fire('destruct::drawPolygon');
                    break;
                case "drawCircle":
                    this.uMap.fire('destruct::drawCircle');
                    break;
            }
        }
        switch (drawMode) {
            ...
            case "drawCircle":
                ...
                break;
            case "measure2":
                this._measureTool2(callback);
                break;
            case "editGeometry":
                this._editGeomtry(callback, obj, state);
                break;
            default:
                break;
        }
    ...
    _editGeomtry: function(callback, obj, state) {
        if (state === 'on') {
            obj.getSelf().editing.enable();
        } else {
            obj.getSelf().editing.disable();
            callback(obj);
        }
    },
    ...
    _drawPoint: function(callback) {
        ...
        var that = this;
        this.modeStatus = "drawPoint";
        this.uMap.on("click", _drawPoint);
        this.uMap.on("destruct::drawPoint", _destruct);
        ...
        function _drawPoint(e) {
            ...
            that.uMap.off("destruct::drawPoint", _destruct);
            that.modeStatus = undefined;
            output(drawPointLatLng);
        }

        function _destruct() {
            that.off('click', _drawPoint);
            that.modeStatus = undefined;
        }
    },

    _measureTool2: function(callback) {
        var measureTool2Handler = new EzServerClient.Polyline.Measure(this.uMap, {});
        measureTool2Handler.setCallBackFunction(callback);
        measureTool2Handler.enable.call(measureTool2Handler);
    },
    ...
    _measureTool: function(callback) {
        ...
        function _measureContextmenu() {
            ...
            that.uMap.off("destruct::measure", _destruct);
            that.modeStatus = undefined;
    ...
    _drawPolyline: function(callback) {
        ...
        _poly.addTo(this.uMap);
        var that = this;
        this.modeStatus = "drawPolyline";
        this.uMap.on("destruct::drawPolyline", _destruct);
        ...
        function _measureContextmenu() {
            ...
            _poly.spliceLatLngs(index, 1);
            that.uMap.off("contextmenu", _measureContextmenu);
            that.uMap.off("destruct::drawPolyline");
            that.modeStatus = undefined;
            if (output !== undefined) {
                var latlngs = _poly.getLatLngs();
                var coordString = '';
    ...
    _drawRect: function(callback) {
        ...
        this.modeStatus = "drawRect";
        this.uMap.on("destruct::drawRect", _destruct);
        ...
        function _drawRectUp(e) {
            ...
            that.uMap.off("destruct::drawRect");
            that.modeStatus = undefined;
    ...
    _drawPolygon: function(callback) {
        ...
        this.uMap.on("destruct::drawPolygon", _destruct);
        ...
        function _drawPolygonContextmenu(e) {
            ...
            that.uMap.off("destruct::drawPolygon");
            that.modeStatus = undefined;
            if (output !== undefined) {
                var latlngs = _poly.getLatLngs();
                var coordString = '';
    ...
    _drawCircle: function(callback) {
        ...
        var _circle = null;
        var that = this;
        this.modeStatus = "drawCircle";
        this.uMap.on("destruct::drawCircle", _destruct);
        ...
        function _drawCircleUp(e) {
            ...
            that.uMap.off("destruct::drawCircle");
            that.modeStatus = undefined;
            that.uMap.dragging.enable();
            if (output !== undefined) {
                _circle.getSelf = function() {
                    return this; };
                output(_circle);
            }
        }
    ...
    //锟斤拷锟斤拷锟斤拷图锟较的伙拷锟狡和硷拷锟斤拷锟侥讹拷锟斤拷
    clear: function() {
        this.uMap.eachLayer(function(layer) {
    ...
    //锟斤拷锟斤拷锟斤拷锟秸空硷拷锟斤拷询锟斤拷锟斤拷锟斤拷锟斤拷
    clearLayers: function() {

    },

    //锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟叫的碉拷图状态锟戒换锟斤拷息锟铰硷拷
    clearMapChangeListener: function() {

    },

    //锟斤拷锟秸碉拷图锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟皆讹拷锟斤拷锟铰硷拷锟斤拷锟斤拷
    clearMapEventListeners: function() {
        this.uMap.clearAllEventListeners();
    },

    //锟斤拷锟秸碉拷图锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷指锟斤拷锟铰硷拷锟斤拷锟斤拷锟铰硷拷锟斤拷锟斤拷
    clearMapInstanceEventListeners: function(eventType) {
        this.uMap.removeEventListener(eventType);
    },

    //锟节碉拷图锟斤拷删锟斤拷锟斤拷锟叫的碉拷锟接讹拷锟斤拷
    clearOverlays: function(bForcedRemove) {

    },

    //锟截憋拷EzMap::openInfoWindow锟斤拷锟津开碉拷锟斤拷息锟斤拷 
    closeInfoWindow: function(popup) {
        this.uMap.closePopup(popup);
    },

    //锟节碉拷锟斤拷图前锟斤拷锟斤拷锟铰ｏ拷锟斤拷锟斤拷图锟斤拷锟斤拷锟叫碉拷锟斤拷锟斤拷锟斤拷锟斤拷值转锟斤拷锟缴碉拷图锟斤拷锟斤拷 
    ...
    //取锟斤拷锟斤拷图锟斤拷锟斤拷锟斤拷锟街凤拷锟斤拷时锟斤拷锟斤拷锟斤拷位锟斤拷为锟斤拷锟侥凤拷锟斤拷效锟斤拷锟斤拷锟斤拷锟皆碉拷图锟斤拷锟斤拷位锟矫凤拷锟斤拷原锟斤拷
    disableMouseZoom: function() {
        this.uMap.scrollWheelZoom.disable();
    },

    //锟斤拷锟斤拷锟较讹拷锟斤拷锟斤拷为锟斤拷锟斤拷锟斤拷
    disableSlipPan: function() {
        this.uMap.dragging.disable();
    },

    //通锟斤拷锟斤拷锟斤拷图片锟侥撅拷锟轿凤拷围锟斤拷锟斤拷锟斤拷锟斤拷图片锟斤拷锟截凤拷式锟斤拷锟斤拷锟截碉拷图图片,锟斤拷锟斤拷锟斤拷锟斤拷一锟斤拷png图片锟斤拷锟斤拷锟斤拷锟截硷拷锟斤拷锟酵碉拷图锟斤拷围锟斤拷锟斤拷一锟斤拷锟竭达拷锟斤拷锟斤拷时锟斤拷一锟斤拷为锟斤拷锟斤拷锟斤拷幕锟竭达拷时锟斤拷锟剿癸拷锟斤拷锟斤拷效锟斤拷
    downloadMap: function(minx, miny, maxx, maxy, iLevel, format) {
        alert("锟睫达拷锟诫！");
    },

    //通锟斤拷矢锟斤拷锟斤拷图锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷源锟侥编辑锟斤拷锟斤拷要锟斤拷EzMapAPI.js锟叫伙拷锟斤拷锟斤拷锟斤拷EditObject::serviceSource锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷EzMapService锟斤拷锟斤拷锟斤拷
    //EzMapService锟斤拷锟斤拷锟斤拷锟斤拷锟侥碉拷锟矫凤拷锟斤拷
    edit: function(pEdit, callBack) {
        try {
            if (!EzServerClient.GlobeFunction.isTypeRight(pEdit, "EditObject") && !EzServerClient.GlobeFunction.isTypeRight(pEdit, "Array")) {
                throw EzErrorFactory.createError("EzMap::pEdit:arguments[0]:error");
    ...
    //锟斤拷锟矫碉拷前锟斤拷图锟斤拷锟斤拷锟竭ｏ拷1:***锟斤拷锟斤拷锟斤拷锟界：1:50000 
    //锟斤拷知锟斤拷锟斤拷leaflet没锟斤拷zoom锟斤拷应锟斤拷
    getCurrentMapScale: function() {

    },

    //锟斤拷锟矫达拷锟斤拷锟斤拷锟斤拷锟铰的匡拷锟饺ｏ拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷为1000 锟阶ｏ拷锟斤拷锟斤拷锟斤拷锟斤拷锟捷革拷锟斤拷锟斤拷锟节碉拷锟斤拷锟较碉拷位锟矫凤拷锟斤拷一锟斤拷锟斤拷锟斤拷值锟斤拷锟斤拷位为锟饺ｏ拷
    ...
    //锟斤拷取锟斤拷锟斤拷EzMapService锟斤拷锟斤拷锟斤拷锟斤拷锟皆达拷锟斤拷锟斤拷默锟斤拷锟斤拷锟皆达拷锟斤拷为1锟斤拷
    //锟斤拷知锟斤拷锟斤拷锟皆后补筹拷
    getEzMapServiceTryTimes: function() {

    },

    //锟斤拷取指锟斤拷锟侥凤拷围锟侥硷拷锟斤拷
    //锟斤拷知锟斤拷,锟斤拷太锟斤拷锟斤拷锟斤拷锟斤拷锟杰碉拷锟斤拷思
    getLevelOfMBR: function(dInMinX, dInMinY, dInMaxX, dInMaxY) {

    },

    //锟斤拷锟斤拷转锟截碉拷图锟斤拷锟斤拷锟斤拷
    ...
    //锟斤拷锟矫撅拷纬锟斤拷锟斤拷锟斤拷锟铰的匡拷锟饺ｏ拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷为0.01 锟饺ｏ拷锟斤拷锟斤拷锟斤拷锟斤拷锟捷革拷锟斤拷锟斤拷锟节碉拷锟斤拷锟较碉拷位锟矫凤拷锟斤拷一锟斤拷锟斤拷锟斤拷值锟斤拷锟斤拷位为锟阶ｏ拷
    //锟斤拷知锟斤拷,锟斤拷太锟斤拷锟斤拷锟斤拷锟斤拷锟杰碉拷锟斤拷思
    getMeter: function(point, degree) {

    },

    //锟斤拷锟斤拷锟斤拷锟斤拷锟节碉拷图锟斤拷X锟斤拷锟斤拷锟较碉拷锟斤拷锟斤拷值锟斤拷锟斤拷位为锟斤拷图默锟较碉拷位锟斤拷
    //锟斤拷知锟斤拷锟斤拷锟斤拷锟斤拷应锟矫癸拷锟斤拷锟斤拷锟斤拷锟铰硷拷
    getMouseMapX: function() {

    },

    //锟斤拷锟斤拷锟斤拷锟斤拷锟节碉拷图锟斤拷Y锟斤拷锟斤拷锟较碉拷锟斤拷锟斤拷值锟斤拷锟斤拷位为锟斤拷图默锟较碉拷位锟斤拷
    //锟斤拷知锟斤拷锟斤拷锟斤拷锟斤拷应锟矫癸拷锟斤拷锟斤拷锟斤拷锟铰硷拷
    getMouseMapY: function() {

    },

    //锟斤拷取锟斤拷图锟斤拷锟矫伙拷锟斤拷锟接的硷拷锟轿讹拷锟襟，憋拷注锟斤拷锟斤拷锟斤拷专锟斤拷图锟斤拷锟斤拷
    getOverlays: function() {
        return this.uMap.getPanes().overlayPane.childNodes;
    },

    //锟斤拷锟矫诧拷询锟斤拷锟斤拷锟斤拷锟较的匡拷隆
    getQueryResult: function() {
    ...
    //锟斤拷示锟斤拷锟叫的碉拷图锟芥本锟斤拷锟斤拷锟叫憋拷锟斤拷使锟斤拷","锟脚分革拷
    ...
    //锟斤拷锟截碉拷图锟斤拷锟斤拷锟截硷拷锟斤拷锟斤拷锟斤拷一锟斤拷锟斤拷图锟截硷拷锟斤拷锟津化碉拷图锟截硷拷锟酵憋拷准锟斤拷图锟截硷拷
    ...
    //锟斤拷锟截憋拷锟斤拷锟斤拷锟斤拷息
    //锟斤拷知锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷1:project锟斤拷锟斤拷
    hideMapScale: function() {

    },

    //锟斤拷锟截碉拷图锟斤拷锟较斤拷锟较角碉拷图锟斤拷锟斤拷锟斤拷钮
    //锟斤拷知锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷
    ...
    //锟节碉拷锟斤拷图前锟斤拷锟斤拷锟铰ｏ拷锟斤拷锟斤拷图锟斤拷锟疥换锟缴碉拷图锟斤拷锟斤拷锟叫碉拷锟斤拷锟斤拷锟斤拷锟斤拷
    mapCoord2Container: function(point) {
        return this.uMap.latLngToContainerPoint(point);
    },

    //锟斤拷锟矫碉拷图锟斤拷锟斤拷状态为锟斤拷锟斤拷锟斤拷锟斤拷状态
    //锟截碉拷锟斤拷锟斤拷原锟酵ｏ拷callback(vStrMeasureArea)
    //vStrMeasureArea {String} : 锟斤拷锟斤拷锟斤拷锟截诧拷锟斤拷锟斤拷锟斤拷锟斤拷
    measureArea: function(data) {
        if (!(data instanceof EzServerClient.Polygon)) {
            return; }
    ...
    //锟斤拷锟矫碉拷图锟斤拷锟斤拷状态为锟斤拷锟斤拷状态
    measureLength: function(data) {
        if (!(data instanceof EzServerClient.Polyline)) {
            return; }
    ...
    //锟节碉拷图指锟斤拷位锟斤拷锟斤拷示锟斤拷息
    //锟斤拷知锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷
    openInfoWindow: function(point, strHTML, bIsInScreen) {

    },

    //通锟斤拷矢锟斤拷锟斤拷图锟斤拷锟斤拷锟斤拷锟叫诧拷询锟斤拷锟界：锟秸硷拷锟斤拷询锟斤拷锟斤拷锟皆诧拷询锟斤拷
    query: function(queryObj, callback) {
        this.uMap.query(queryObj, callback);
    },

    //锟斤拷锟叫碉拷指锟斤拷锟侥点，锟斤拷锟斤拷锟矫碉拷锟节碉拷前锟斤拷图锟较ｏ拷锟斤拷锟斤拷锟斤拷平锟狡碉拷锟矫碉拷为锟斤拷图锟斤拷锟斤拷
    ...
    //锟节碉拷图锟斤拷删锟斤拷指锟斤拷锟侥碉拷锟接讹拷锟斤拷
    removeOverlay: function(overlay) {
        this.uMap.removeLayer(overlay.getSelf());
        if (overlay instanceof Polyline) {
            if (overlay.arrowPoly_) {
                this.uMap.removeLayer(overlay.arrowPoly_);
            }
        }
    },

    //锟斤拷锟矫碉拷图锟斤拷锟斤拷锟叫帮拷锟斤拷锟侥空硷拷锟斤拷锟斤拷图锟斤拷
    setEzLayer: function(pEzLayerArr) {

    },

    //锟斤拷锟矫碉拷锟斤拷EzMapService锟斤拷锟斤拷时锟斤拷锟斤拷锟皆达拷锟斤拷锟斤拷默锟斤拷锟斤拷锟皆达拷锟斤拷为1锟斤拷
    setEzMapServiceTryTimes: function(vTimes) {
        this.uMap.setEzMapServiceTryTimes(vTimes);
    },

    //锟斤拷示锟斤拷权锟斤拷息锟节碉拷图锟截硷拷锟斤拷锟斤拷锟铰斤拷
    showCopyright: function() {
        this.uMap.attributionControl.options.prefix = this._prefixAttributions;
        this.uMap.attributionControl._update();
    },

    //锟斤拷示锟斤拷锟斤拷锟斤拷锟斤拷息锟节碉拷图锟斤拷锟斤拷锟铰斤拷
    //锟斤拷知锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷
    showMapScale: function() {

    },

    //锟斤拷示锟斤拷图锟斤拷锟较斤拷锟较角碉拷图锟斤拷锟斤拷锟斤拷钮
    //锟斤拷知锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷
    showMapServer: function() {

    },

    //锟斤拷示锟津化碉拷图锟截硷拷锟斤拷锟津化碉拷图锟斤拷锟狡控硷拷锟斤拷锟斤拷锟斤拷锟脚达拷一锟斤拷锟斤拷锟斤拷锟斤拷钮锟斤拷锟斤拷小一锟斤拷锟斤拷锟斤拷锟斤拷钮锟斤拷锟斤拷锟较★拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟铰和讹拷锟叫帮拷钮
    ...
    //锟斤拷示锟斤拷准锟斤拷图锟截硷拷锟斤拷锟斤拷准锟斤拷图锟斤拷锟狡控硷拷锟斤拷锟斤拷锟斤拷小锟斤拷图锟斤拷锟狡控硷拷锟斤拷锟较革拷锟斤拷锟斤拷锟斤拷锟斤拷钮
    ...
    //锟斤拷图锟斤拷指锟斤拷锟斤拷锟斤拷锟叫凤拷锟斤拷锟斤拷锟斤拷锟剿碉拷锟节碉拷图锟斤拷锟脚癸拷锟斤拷锟斤拷位锟矫诧拷锟斤拷
    zoomAtPoint: function(oPoint, nZoomLevel) {
        this.uMap.setView(oPoint, nZoomLevel);
    },

    //通锟斤拷锟矫伙拷锟节碉拷图锟较斤拷锟斤拷锟侥伙拷锟狡撅拷锟轿ｏ拷锟皆碉拷图锟斤拷锟叫放大。猴拷锟斤拷执锟叫猴拷锟斤拷原始锟斤拷锟斤拷锟斤拷锟斤拷失去锟斤拷锟姐，锟斤拷锟斤拷锟斤拷为锟斤拷图锟截硷拷
    zoomInExt: function() {
        this.uMap.dragging.disable();
    ...
    _dragRectMove: function(e) {
        if (!this.topLeftOfBounds || this.topLeftOfBounds === null) {
            return; }
    ...
    //通锟斤拷锟矫伙拷锟节碉拷图锟较斤拷锟斤拷锟侥伙拷锟狡撅拷锟轿ｏ拷锟皆碉拷图锟斤拷锟斤拷锟斤拷小锟斤拷锟斤拷锟斤拷执锟叫猴拷锟斤拷原始锟斤拷锟斤拷锟斤拷锟斤拷失去锟斤拷锟姐，锟斤拷锟斤拷锟斤拷为锟斤拷图锟截硷拷
    zoomOutExt: function() {
        this.uMap.dragging.disable();
...
Icon = EzServerClient.Icon.extend({
	...
	update:function(){
		var options = {
			"iconUrl": this.image,
			"iconSize": new EzServerClient.Point(this.width, this.height),
...
Marker = EzServerClient.Class.extend({
	initialize: function(point, icon, title){
		...
		if (uicon !== undefined) {
			...
			if (options !== undefined) {
				newOptions = options;
				newOptions['icon'] = uicon;
			} else {
				newOptions = {
					'icon': uicon
				};
			}
			this.uMarker = EzServerClient.marker(latlng, newOptions);
		} else {
	...
	addTo:function(map){
		this.uMarker['markerFlag'] = true;
		this.uMarker.addTo(map);
	},

	flash: function(times) {
		this.uMarker.flash(times);
	},

	setZIndex: function(offset){
		this.uMarker.setZIndexOffset(offset);
	}
});

HTMLElementOverLay = EzServerClient.Class.extend({
	initialize: function(id, LeftTopMapPoint, strHTML,popupAnchor,iconAnchor,draggable) {
		var popupAn = popupAnchor?popupAnchor:[0,0],
			iconAn = iconAnchor?iconAnchor:[0,0];
		var divicon = EzServerClient.divIcon({
			className: id,
			html: strHTML,
			iconSize: null,
			popupAnchor: new EzServerClient.Point(popupAn[0],popupAn[1]),
			iconAnchor: new EzServerClient.Point(iconAn[0],iconAn[1])
		});

		var _latlng = EzServerClient.latLng(LeftTopMapPoint.y, LeftTopMapPoint.x);
		this._marker = EzServerClient.marker(_latlng,{icon:divicon,draggable:draggable});
	},

	addTo: function(map) {
		this._marker['htmlelementoverlayFlag'] = true;
		this._marker.addTo(map);
	},

	openInfoWindowHtml: function(strHTML,options){
		if (!this._marker.getPopup()) {
			this._marker.bindPopup(strHTML,options);
...
/**
 * [QueryItem description] 锟斤拷锟角诧拷询锟斤拷
 * @type {[type]}
 */
EzServerClient.EasyXmlParse.QueryItem = EzServerClient.EasyXmlParse.extend({
	options: {
		timeOut: null, //锟斤拷询锟斤拷时时锟斤拷 锟斤拷始默锟斤拷值锟斤拷锟斤拷
		tryTime: null, //锟斤拷询锟斤拷时时锟斤拷 锟斤拷始默锟斤拷值锟斤拷锟斤拷
		state: State.SUCCESS, //锟斤拷锟矫碉拷默锟较碉拷锟接诧拷询状态 锟斤拷始默锟斤拷值锟斤拷true
		failureOnError: false //锟斤拷询锟斤拷锟斤拷锟角诧拷锟斤拷一锟斤拷锟斤拷锟斤拷 锟斤拷始默锟斤拷值锟斤拷false
	},
	...
	initialize: function (url, xmlDoc, options) {
		...
		this.currentTryTime = 0; //锟斤拷前锟斤拷询锟斤拷锟斤拷锟斤拷锟斤拷 锟斤拷始默锟斤拷值锟斤拷0
		this.reSendId = 0; //锟斤拷锟斤拷ID 锟斤拷始默锟斤拷值锟斤拷0
		this.responseXml = null; //锟斤拷锟截碉拷Xml 锟斤拷始默锟斤拷值锟斤拷锟斤拷
	},

	/**
	 * 锟斤拷锟斤拷锟斤拷锟杰ｏ拷锟斤拷锟矫筹拷时时锟斤拷
	 *
	 * @param {Int}
	 *            timeOut 锟斤拷锟斤拷锟斤拷锟藉：锟斤拷时时锟斤拷
	 * @return {void}
	 */
	...
	/**
	 * 锟斤拷锟斤拷锟斤拷锟杰ｏ拷锟斤拷锟斤拷锟斤拷锟皆的达拷锟斤拷
	 *
	 * @param {Int}
	 *            tryTime 锟斤拷锟斤拷锟斤拷锟藉：锟斤拷锟皆达拷锟斤拷
	 * @return {void}
	 */
	...
	/**
	 * 锟斤拷锟斤拷锟斤拷锟杰ｏ拷锟斤拷取锟斤拷锟截斤拷锟斤拷状态锟斤拷锟斤拷锟截诧拷询锟斤拷锟斤拷锟角凤拷为一锟斤拷锟斤拷
	 *
	 * @return {boolean}
	 */
	...
	doQuery: function () {
		...
		/* 锟斤拷锟斤拷锟接诧拷询没锟斤拷锟斤拷锟皆ｏ拷锟斤拷锟斤拷锟斤拷锟斤拷为NextOrder锟斤拷锟接诧拷询锟斤拷锟斤拷锟斤拷锟斤拷锟叫憋拷锟斤拷 */
		...
		/* 锟斤拷锟斤拷锟接诧拷询锟斤拷锟皆ｏ拷锟斤拷锟斤拷锟斤拷锟皆碉拷ID锟斤拷pMe.reSendId锟斤拷锟接诧拷询锟斤拷锟斤拷原锟叫碉拷锟斤拷锟斤拷锟斤拷元锟斤拷 */
		else
			RequestMapObject.add(xmlhttp, new RequestMapItemBinding(pMe.reSendId,
				text2Send));
		xmlhttp.send(text2Send);

		/* 只锟斤拷没锟斤拷锟斤拷锟皆ｏ拷EzServerClient.EasyXmlParse.EzServerClient.EasyXmlParse.EzMapserviceQuery.NextOrder++锟斤拷锟斤拷锟斤拷 */
		if (pMe.currentTryTime == 0)
			EzServerClient.EasyXmlParse.EzMapserviceQuery.NextOrder++;
		...
		function HandleStateChange() {
			if (xmlhttp.readyState == 4) {
				...
				if (xmlhttp.status == 200) {
					...
					//doc1.async = false;
					//doc1.loadXML(xmlhttp.responseText);  //锟斤拷锟截凤拷锟截碉拷xml
					if ((doc1.getElementsByTagName("ERROR").length == 0)) { //锟斤拷锟截碉拷xml没锟叫达拷锟斤拷锟斤拷息
						var o = RequestMapObject.getValue(xmlhttp);
						RequestMapObject.remove(xmlhttp);
						pMe.setResponseXml(xmlhttp.responseText); //锟斤拷锟矫凤拷锟斤拷锟斤拷息
						pMe.setQueryXml(RequestMapObject.getValue(xmlhttp).text); //锟斤拷取锟斤拷锟酵碉拷xml锟斤拷锟斤拷锟斤拷
						pMe.itemId = RequestMapObject.getValue(xmlhttp).num; //锟斤拷取锟斤拷锟酵碉拷xml锟斤拷锟斤拷锟斤拷
						if (--pMe.mapServiceQuery.options.itemCurrentCount == 0) { //锟斤拷锟斤拷锟接诧拷询锟斤拷锟斤拷锟缴碉拷锟斤拷锟斤拷
							EzServerClient.EasyXmlParse.EzMapserviceQuery.queryState = false; //锟斤拷锟斤拷锟斤拷时锟斤拷锟斤拷锟斤拷询锟斤拷锟斤拷锟窖撅拷锟斤拷锟斤拷
							pMe.mapServiceQuery.onFinished();
						}
					} else { //锟斤拷锟截碉拷xml锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷息
						if (++pMe.currentTryTime > pMe.getTryTime()) { //锟叫断达拷时锟斤拷前锟截诧拷锟斤拷锟斤拷锟角否超癸拷锟借定锟斤拷锟截诧拷锟斤拷锟斤拷
							pMe.setResultState(State.ERROR); //锟斤拷锟斤拷锟接诧拷询锟斤拷锟斤拷状态
							pMe.mapServiceQuery.options.queryFlag++;
							--pMe.mapServiceQuery.options.itemCurrentCount;
							if (pMe.mapServiceQuery.options.failureOnError //锟矫诧拷询锟斤拷锟斤拷锟斤拷一锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟接诧拷询锟叫筹拷锟斤拷锟斤拷直锟斤拷锟阶达拷
								&& pMe.mapServiceQuery.options.queryFlag == 1) {
								EzServerClient.EasyXmlParse.EzMapserviceQuery.queryState = false;
								throw new Error("锟斤拷锟节碉拷" + RequestMapObject.getValue(xmlhttp).num + "锟斤拷锟接诧拷询锟斤拷锟斤拷锟斤拷锟矫诧拷询锟斤拷锟斤拷锟斤拷止");
							}
							pMe.setResponseXml(xmlhttp.responseText); //锟斤拷锟矫凤拷锟斤拷锟斤拷息
							if (pMe.mapServiceQuery.options.itemCurrentCount == 0) { //锟接诧拷询锟窖撅拷锟斤拷锟斤拷锟剿ｏ拷锟斤拷锟节斤拷锟斤拷锟斤拷询锟斤拷锟教ｏ拷锟斤拷锟斤拷onFinished()锟斤拷锟教ｏ拷锟斤拷锟斤拷锟矫伙拷锟斤拷取锟斤拷锟斤拷要锟斤拷锟斤拷息
								EzServerClient.EasyXmlParse.EzMapserviceQuery.queryState = false;
								pMe.mapServiceQuery.onFinished();
							}
							return;
				...
				} else { //锟斤拷锟斤拷锟斤拷息锟斤拷实200锟斤拷锟斤拷锟斤拷状态锟斤拷锟矫诧拷锟斤拷锟斤拷确锟斤拷xml锟斤拷息锟斤拷锟斤拷锟斤拷锟接诧拷询error,锟斤拷锟斤拷锟结返锟截革拷锟矫伙拷锟斤拷息(null)
...
EzServerClient.EasyXmlParse.E_Element.E_Shape = EzServerClient.EasyXmlParse.E_Element.extend({
	...
	fromElement: function(vSrcElem) {
		// 濡傛灉缁欏叆鐨刋ML瀵硅薄涓嶅瓨鍦紝灏辫繑鍥炵┖瀵硅薄涓庝箣瀵瑰簲
		if (vSrcElem == null)
			return null;
		// 鑺傜偣瀛樺湪鎯呭喌涓嬭В鏋�
...
EzServerClient.EasyXmlParse.E_Execute = EzServerClient.EasyXmlParse.extend({
	 ...
	 * 
	 * <P>
	 * <font color="red"><B>娉ㄦ剰锛�<br>
	 * DeleteQuery,InsertQuery,SelectQuery,UpdateQuery浠讳綍涓€涓璞￠兘鍙互缃┖<br>
	 * 浣嗘槸锛屼粬浠箣涓繀椤绘湁涓斾粎鏈変竴涓笉鏄┖<br>
	 * 璁剧疆浠绘剰4鑰呬箣涓€涓洪潪绌猴紝浼氬鑷村叾瀹�3鑰呰嚜鍔ㄦ竻绌� </B></font>
	 * </P>
	 * 
	 * @param selectQuery
	 *            鏂扮殑鏌ヨ瀵硅薄
...
EzServerClient.EasyXmlParse.E_EasyXml = EzServerClient.EasyXmlParse.extend({
	...
	setVersion:function(version){
		...
		if (EzServerClient.EasyXmlParse.E_EasyXml.version != "1.1")
			throw new Error("璁剧疆鐨勭増鏈彿涓嶆槸EasyXML1.1");
	},
...
EzServerClient.EasyXmlParse.EzFactory = EzServerClient.EasyXmlParse.extend({
	statics:{
		/**
		 * 姝ょ▼搴忔槸灏嗗鎴风璇锋眰鐨凟zXml瀵硅薄杞崲鎴怷ml
		 */
		ToXml:function(EasyXml){
			if (window.ActiveXObject) {
				...
				return new XMLSerializer().serializeToString(oXmlDom); //杩欓噷鏄徃鏈涘埄鎶樿吘鍑烘潵鐨勶紒锛侊紒              
			} else {
		 ...
		 * @date qianleyi 2014.8.21
		 * @description 姝ょ▼搴忔槸灏嗘湇鍔″櫒杩斿洖鐨刋ml杞崲鎴怑zXml1.1璇█鑳藉璇嗗埆鐨勫璞�
		 */
	...
	statics:{
		...
		getChildElement:function(vElem,vTagName){
			...
			if (tmpList.length == 0)
				...
				// 鑾峰緱鐨勮妭鐐癸紝鍙兘涓嶆槸鏈骇鐨勶紝鏄瓙鑺傜偣鐨勫悓鍚嶆爣绛�
...
EzServerClient.EasyXmlParse.E_Error = EzServerClient.EasyXmlParse.extend({
	...
	fromElement:function(vSrcElem){
		// 锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷XML锟斤拷锟襟不达拷锟节ｏ拷锟酵凤拷锟截空讹拷锟斤拷锟斤拷之锟斤拷应
		if (vSrcElem == null)
			return null;
		// 锟节碉拷锟斤拷锟斤拷锟斤拷锟斤拷锟铰斤拷锟斤拷
	...
	setMessage:function(message){
		if (message == null) {
			this.message = "锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷息锟斤拷锟捷ｏ拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷";
		} else {
...
/**
 * @description [锟斤拷锟斤拷锟斤拷询锟斤拷]
 * 
 */
EzServerClient.EasyXmlParse.EzMapserviceQuery = EzServerClient.EasyXmlParse.extend({
	//锟斤拷锟斤拷
	options: {
		timeOut: null, 											//锟斤拷锟矫的筹拷时时锟斤拷 锟斤拷始默锟斤拷值锟斤拷2000锟斤拷锟斤拷
		tryTime: 3, 											//锟斤拷锟斤拷锟接诧拷询锟斤拷锟皆达拷锟斤拷 锟斤拷始默锟斤拷值锟斤拷3
		failureOnError: false, 									//锟斤拷询锟斤拷锟斤拷锟角诧拷锟斤拷锟斤拷锟斤拷 锟斤拷始默锟斤拷值锟斤拷false		
		itemCurrentCount: null, 								//锟斤拷前未锟缴癸拷锟斤拷询锟斤拷锟接诧拷询锟斤拷锟斤拷锟斤拷锟斤拷 锟斤拷始默锟斤拷值锟斤拷锟斤拷
		queryFlag: 0 											//锟斤拷锟矫碉拷一锟斤拷锟斤拷询锟斤拷锟斤拷锟侥筹拷锟斤拷锟斤拷锟斤拷
	},

	statics:{
		queryState: false, 										//锟斤拷态锟斤拷锟斤拷锟皆ｏ拷锟斤拷询锟斤拷锟斤拷锟斤拷状态锟斤拷默锟斤拷锟斤拷false,锟斤拷锟节诧拷询
		NextOrder: 0									//锟斤拷态锟斤拷锟斤拷锟皆ｏ拷锟斤拷锟矫的诧拷询锟斤拷全锟街憋拷锟斤拷锟斤拷锟斤拷录锟窖撅拷锟斤拷锟酵的诧拷询锟斤拷锟接讹拷锟斤拷 默锟较筹拷始值锟斤拷0
	},

	initialize:function(options){
		EzServerClient.setOptions(this, options);

		this.queryItem = new Array(); 							//锟斤拷询锟接讹拷锟斤拷锟斤拷锟斤拷 锟斤拷始默锟斤拷值锟斤拷锟斤拷
		this.itemCount = null;									//锟斤拷录锟接诧拷询锟侥革拷锟斤拷
	},

	/**
	 * @description [锟斤拷锟斤拷锟接诧拷询锟斤拷锟斤拷]
	 * @param {Object}
	 *            queryItem 锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟接诧拷询 锟斤拷锟斤拷省锟皆ｏ拷锟斤拷锟斤拷省锟斤拷
	 * @return {void}
	 */
	addQueryItem: function(queryItem) {
		/* 锟斤拷锟斤拷锟斤拷锟斤拷实锟斤拷锟斤拷锟斤拷锟斤拷目锟斤拷锟斤拷锟斤拷锟接诧拷询锟斤拷锟杰癸拷锟矫碉拷锟斤拷询锟斤拷锟斤拷 */
	...
	/**
	 * @description 锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟矫诧拷询锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷
	 * @param {Int}
	 *            tryTime 锟斤拷锟斤拷锟斤拷锟藉：锟斤拷锟皆达拷锟斤拷
	 * @return {void}
	 */
	...
	/**
	 * @description 锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷一锟斤拷锟斤拷询锟斤拷锟斤拷锟斤拷锟角诧拷锟斤拷一锟斤拷锟斤拷锟斤，默锟斤拷锟斤拷锟斤拷为False
	 * @param {Boolean}
	 *            failureOnError 锟斤拷锟斤拷锟斤拷锟斤拷:锟斤拷询锟斤拷锟斤拷锟角凤拷锟斤拷锟斤拷锟斤拷
	 * @return {void}
	 */
	...
	/**
	 * @description 锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷 执锟叫诧拷询锟斤拷锟斤拷
	 * @return {void}
	 */
	doQuery:function(){
		/* 锟斤拷锟斤拷锟斤拷前锟侥诧拷询状态为false,锟斤拷锟斤拷锟斤拷为true,锟斤拷示锟斤拷锟节斤拷锟叫诧拷询 */
		if (EzServerClient.EasyXmlParse.EzMapserviceQuery.queryState == false)
			...
			throw new Error("EzServerClient.EasyXmlParse.EzMapserviceQuery::doQuery锟斤拷询锟斤拷没锟叫诧拷询锟斤拷锟斤拷锟斤拷锟斤拷锟杰斤拷锟斤拷锟铰次诧拷询");
		}

		/* 锟斤拷锟矫的诧拷询未锟缴癸拷锟侥革拷锟斤拷 */
		this.options.itemCurrentCount = this.itemCount;

		/* 锟斤拷询每一锟斤拷锟接诧拷询 */
		for ( var i = 0; i < this.queryItem.length; i++) {

			/* 锟斤拷锟矫伙拷锟斤拷锟矫的诧拷询锟斤拷锟斤拷锟侥革拷锟斤拷锟斤拷锟皆达拷锟诫到每一锟斤拷锟斤拷询锟接讹拷锟斤拷锟斤拷去锟斤拷锟斤拷锟斤拷每一锟斤拷锟斤拷锟斤拷锟叫碉拷默锟斤拷值 */
...
/**
 * HotSpot.js澧炲姞Map浜嬩欢锛屽垽鏂槸鍚otSpot鏄剧ず
 */
EzServerClient.Map.include({
...
/**
 * EzServerClient.HotSpot绫诲畾涔塇otSpot鏍峰紡绛�
 */
...
/**
 * HotSpot.js 鍩轰簬GeoJSON鏁版嵁鏍煎紡鐨凥otSpot瑙ｅ喅鏂规
 */
EzServerClient.HotSpot = EzServerClient.Class.extend({
	initialize: function() {
...
EzServerClient.Symbol = EzServerClient.Symbol || {};


EzServerClient.Symbol.Dash = EzServerClient.Class.extend({
    ...
    buildSymbol: function(dirPoint, latLngs, map, index, total) {
        ...
        if (opts.pixelSize <= 1) {
            return new EzServerClient.Polyline([dirPoint.latLng, dirPoint.latLng], opts.pathOptions);
        }

        var midPoint = map.project(dirPoint.latLng);
        var angle = (-(dirPoint.heading - 90)) * d2r;
        var a = new EzServerClient.Point(
            midPoint.x + opts.pixelSize * Math.cos(angle + Math.PI) / 2,
            midPoint.y + opts.pixelSize * Math.sin(angle) / 2
        );
        // compute second point by central symmetry to avoid unecessary cos/sin
        var b = midPoint.add(midPoint.subtract(a));
        return new EzServerClient.Polyline([map.unproject(a), map.unproject(b)], opts.pathOptions);
    }
});

EzServerClient.Symbol.dash = function(options) {
    return new EzServerClient.Symbol.Dash(options);
};

EzServerClient.Symbol.ArrowHead = EzServerClient.Class.extend({
    isZoomDependant: true,
    ...
    buildSymbol: function(dirPoint, latLngs, map, index, total) {
        ...
        var path;
        if (opts.polygon) {
            path = new EzServerClient.Polygon(this._buildArrowPath(dirPoint, map), opts.pathOptions);
        } else {
            path = new EzServerClient.Polyline(this._buildArrowPath(dirPoint, map), opts.pathOptions);
        }
        return path;
    },
...
EzServerClient.Symbol.arrowHead = function(options) {
    return new EzServerClient.Symbol.ArrowHead(options);
};

EzServerClient.Symbol.Marker = EzServerClient.Class.extend({
    ...
    initialize: function(options) {
        ...
        this.isZoomDependant = (EzServerClient.Browser.ie && this.options.rotate);
    },

    buildSymbol: function(directionPoint, latLngs, map, index, total) {
        if (!this.options.rotate) {
            return new EzServerClient.Marker(directionPoint.latLng, this.options.markerOptions);
        } else {
            this.options.markerOptions.angle = directionPoint.heading;
            return new EzServerClient.RotatedMarker(directionPoint.latLng, this.options.markerOptions);
        }
    }
});

EzServerClient.Symbol.marker = function(options) {
    return new EzServerClient.Symbol.Marker(options);
};


EzServerClient.RotatedMarker = EzServerClient.Marker.extend({
    options: {
        angle: 0
    },

    statics: {
        TRANSFORM_ORIGIN: EzServerClient.DomUtil.testProp(
            ['transformOrigin', 'WebkitTransformOrigin', 'OTransformOrigin', 'MozTransformOrigin', 'msTransformOrigin'])
    },

    _initIcon: function() {
        EzServerClient.Marker.prototype._initIcon.call(this);

        this._icon.style[EzServerClient.RotatedMarker.TRANSFORM_ORIGIN] = '50% 50%';
    },

    _setPos: function (pos) {
        EzServerClient.Marker.prototype._setPos.call(this, pos);

        if (EzServerClient.DomUtil.TRANSFORM) {
            // use the CSS transform rule if available
            this._icon.style[EzServerClient.DomUtil.TRANSFORM] += ' rotate(' + this.options.angle + 'deg)';
        } else if(EzServerClient.Browser.ie) {
...
EzServerClient.rotatedMarker = function (pos, options) {
    return new EzServerClient.RotatedMarker(pos, options);
};



EzServerClient.PolylineDecorator = EzServerClient.LayerGroup.extend({
    options: {
        patterns: []
    },

    initialize: function(paths, options) {
        EzServerClient.LayerGroup.prototype.initialize.call(this);
        EzServerClient.Util.setOptions(this, options);
        this._map = null;
    ...
    */
    _initPaths: function(p) {
        this._paths = [];
        var isPolygon = false;
        if(p instanceof EzServerClient.Polyline) {
            this._initPath(p.getLatLngs(), (p instanceof EzServerClient.Polygon));
        } else if(EzServerClient.Util.isArray(p) && p.length > 0) {
            if(p[0] instanceof EzServerClient.Polyline) {
                for(var i=0; i<p.length; i++) {
                    this._initPath(p[i].getLatLngs(), (p[i] instanceof EzServerClient.Polygon));
                }
            } else {
                this._initPath(p);
            }
    ...
    _isCoordArray: function(ll) {
        return(EzServerClient.Util.isArray(ll) && ll.length > 0 && (
            ll[0] instanceof EzServerClient.LatLng ||
            (EzServerClient.Util.isArray(ll[0]) && ll[0].length == 2 && typeof ll[0][0] === 'number')
        ));
    },
    ...
    */
    ...
    onRemove: function (map) {
        ...
        this._map = null;
        EzServerClient.LayerGroup.prototype.onRemove.call(this, map);
    },
    ...
    */
    _getDirectionPoints: function(pathIndex, pattern) {
        ...
        if(pattern.isOffsetInPixels) {
            pathPixelLength =  EzServerClient.LineUtil.PolylineDecorator.getPixelLength(latLngs, this._map);
            offset = pattern.offset/pathPixelLength;
        } else {
            offset = pattern.offset;
        }
        if(pattern.isEndOffsetInPixels) {
            pathPixelLength = (pathPixelLength !== null) ? pathPixelLength : EzServerClient.LineUtil.PolylineDecorator.getPixelLength(latLngs, this._map);
        ...
        dirPoints = EzServerClient.LineUtil.PolylineDecorator.projectPatternOnPath(latLngs, offset, endOffset, repeat, this._map);
 ...
 */
EzServerClient.polylineDecorator = function (paths, options) {
    return new EzServerClient.PolylineDecorator(paths, options);
};



EzServerClient.LineUtil.PolylineDecorator = {
    ...
    getPixelLength: function(pl, map) {
        var ll = (pl instanceof EzServerClient.Polyline) ? pl.getLatLngs() : pl,
            nbPts = ll.length;
        if(nbPts < 2) {
            return 0;
        }
    ...
    /**
    * path: array of EzServerClient.LatLng
    ...
    */
    ...
    /**
    * pts: array of EzServerClient.Point
    ...
    */
    interpolateBetweenPoints: function (ptA, ptB, ratio) {
        ...
        return new EzServerClient.Point(ptA.x, ptA.y + (ptB.y - ptA.y) * ratio);
    }
};
      ...
      if (objLayer) {
        if (objLayer.options.isEzMap) {
          ...
          for(var i in objMap._layers){
            if (objMap._layers[i] instanceof EzServerClient.TileLayer) {
          ...
          if (objLayer.options.crs.code === 'EPSG:4326') {
            objMap.options.crs = EzServerClient.CRS.EPSG4326;
          } else {
          ...
          objMap._resetView(tempCenter,tempZoom);
          objMap.addLayer(objLayer);
          if(objLayer.options.annotationLayer_){
            objMap.addLayer(objLayer.options.annotationLayer_);
          }
...
/*
 *  Simple navigation control that allows back 锛宖orward 锛宭eft and right navigation through map's view history
 */
...
EzServerClient.Control.Layers = EzServerClient.Control.extend({
	...
	_onInputClick: function () {
		...
		/**
		 * @author [qianleyi]
		 * @description [鍒囨崲鍦板浘锛堜笉鍚屽潗鏍囩郴鐨勶級]
		 */
		var tempCenter=this._map.getCenter();
		var tempZoom=this._map.getZoom();
		var objMap = this._map;
...
EzServerClient.LayersAPI = EzServerClient.Class.extend({
    ...
    changeToLayer: function(name) {
        ...
        for (var i in objMap._layers) {
            if (objMap._layers[i] instanceof EzServerClient.TileLayer) {
                if (objMap._layers[i]._hotspotFlag) continue;
                objMap.removeLayer(objMap._layers[i]);
            }
        }
        ...
        if (!this._map.hasLayer(obj.layer)) {
            if (obj.layer.options.isEzMap) {
                ...
                objMap.addLayer(obj.layer);
                if (obj.layer.options.annotationLayer_) {
                    objMap.addLayer(obj.layer.options.annotationLayer_);
                    obj.layer.options.annotationLayer_.bringToFront();
                }
                if (objMap._hotSpotLayer) {
                    objMap._hotSpotLayer.bringToFront();
                }
            } else {
                objMap.options.crs = obj.layer.options.crs;
                objMap.options.isEzMap = false;
...
EzServerClient.Control.Zoomslider = EzServerClient.Control.extend({
	...
	onAdd: function(map) {
		...
		/**
		 * @author [qianleyi]
		 * @description [缁戝畾浜嬩欢浣垮緱绱姞_refresh锛堬級瀵艰嚧鍐呭瓨娉勯湶]
		 */
...
EzServerClient.Control.MiniMap = EzServerClient.Control.extend({
	 ...
	 * @author qianleyi
	 * @date 2014.9.12
	 * @description minimap鏄惁鏄剧ずoverlay鍥惧眰锛�
	 */
	_refresh:function(e){
		...
		// }
			...
			this._adjustMapParameter(e.layer);
	 ...
	 * @return {[type]} [description]
	 */
	_adjustMapParameter:function(layer){
		...
		if (this._map.options.isEzMap) {
			...
			this._miniMap.options.crs = this._mainMap.options.crs;
